...
type AfterSaleItem struct {
	GoodsSKU    string `json:"goods_sku"`     // 货品 SKU
...
type AfterSale struct {
	...
	BuyerAccountId         string          `json:"buyer_account_id"`          // 买家 ID
	...
	SellerAccountId        string          `json:"seller_account_id"`         // 卖家 ID
...
type AfterSalesQueryParams struct {
	Paging
	ApproveStatus   string `json:"approveStatus"`   // 审核状态（approveStatus：格式错误 、applying：等待提交、approving：等待审批、approved：审批通过）
	...
	if resp.IsSuccess() {
		if err = tongtool.ErrorWrap(res.Code, res.Message); err == nil {
			...
			for i := range items {
				...
				if items[i].AfterSaleServiceType[0:1] == "1" {
					ass.Refunded = true
				}
				if items[i].AfterSaleServiceType[1:2] == "1" {
					ass.ReturnedGoods = true
				}
				if items[i].AfterSaleServiceType[2:3] == "1" {
